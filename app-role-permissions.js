...
(function () {
  ...
  const defaultState = () => ({
    company: { name: "My Business", currency: "PKR", fiscalYearStartMonth: 7 },
    accounts: [
...
// Backward compatible: old entries with no status are treated as approved
function isApprovedTxn(t) { return !t.status || t.status === "approved"; }
function isPendingTxn(t) { return t.status === "pending"; }
function isRejectedTxn(t) { return t.status === "rejected"; }
...
function badgeStatus(status) {
  ...
  if (s === "pending") return `<span class="badge" style="color:#b45309">Pending</span>`;
  if (s === "rejected") return `<span class="badge" style="color:#b91c1c">Rejected</span>`;
  return `<span class="badge" style="color:#15803d">Approved</span>`;
}
          ...
          ${list.map(p => `
            <tr data-id="${p.id}">
              ...
              <td class="row">
                ...
                ${isAdmin() ? `
                  ${t.status === "pending" ? `<button class="btn small" data-act="approve">Approve</button>
  ...
  function bindPartyTable(kind) {
    $("#partyTable").addEventListener("click", (e) => {
      ...
      if (act === "del") {
        if (!isAdmin()) return alert("Manager cannot delete. Only admin can delete.");
  ...
  function renderTxnTable(rows) {
    ...
    return `
      <table class="table" id="txnTable">
        <thead><tr>
          <th>Date</th><th>Type</th><th>Status</th><th>Party</th><th>Ref</th><th>Description</th><th>Category</th><th>Amount</th><th>Paid</th><th>Method</th><th>Actions</th>
        </tr></thead>
        <tbody>
          ${rows.map(t => `
            <tr data-id="${t.id}">
    ...
    <div class="card section">
      <div class="row space">
        <div>
          ...
          <div class="muted tiny">Company: <b>${escapeHtml(state.company.name)}</b></div>
        </div>
...
function recalcAmountFromQtyRate() {
  ...
  if (qty > 0 && rate > 0) {
    ...
    $("#txnTableWrap").addEventListener("click", (e) => {
      ...
      const act = btn.dataset.act;
      if (act === "approve") {
        if (!isAdmin()) return alert("Only admin can approve.");
        const t = state.ledger.find(x => x.id === id);
        if (!t) return;
        t.status = "approved";
        t.approvedBy = session.username;
        t.approvedAt = new Date().toISOString();
        saveCache(state);
        scheduleCloudSave("approve");
        route();
        return;
      }
      if (act === "reject") {
        if (!isAdmin()) return alert("Only admin can reject.");
        const t = state.ledger.find(x => x.id === id);
        if (!t) return;
        t.status = "rejected";
        t.rejectedBy = session.username;
        t.rejectedAt = new Date().toISOString();
        saveCache(state);
        scheduleCloudSave("reject");
        route();
        return;
      }
      if (act === "invoice") {
      ...
      if (act === "edit") {
        if (!isAdmin()) return alert("Manager cannot edit. Only admin can edit.");
        openTxn(id);
      }
      if (act === "del") {
        if (!isAdmin()) return alert("Manager cannot delete. Only admin can delete.");
    ...
    form.addEventListener("submit", (e) => {
      ...
      // Permissions: manager can only create (no edit)
      if (existing && !isAdmin()) {
        alert("Manager cannot edit existing entries. Please ask admin.");
        return;
      }
      // Status: manager entries require approval
      if (!existing) {
        obj.createdBy = session?.username || "unknown";
        obj.createdRole = session?.role || "unknown";
        obj.createdAt = new Date().toISOString();
        obj.status = isAdmin() ? "approved" : "pending";
      } else {
        // admin edit keeps existing status (default approved)
        obj.status = existing.status || "approved";
      }
      if (existing) Object.assign(existing, obj);
